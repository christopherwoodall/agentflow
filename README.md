...
That wrapper now runs both the raw shell-level Kimi toolchain check and the bundled `agentflow toolchain-local`, `inspect`, `doctor`, and `check-local` commands before it moves on to the external custom-pipeline `doctor`, `inspect`, `check-local`, and `run` paths for `bootstrap: kimi`, explicit `shell_init: kimi`, and explicit `target.shell` Kimi wrappers. This keeps the maintainer path closer to the real user-facing debugging flow, so regressions in the bundled inspect or doctor surfaces do not hide behind a still-green `check-local` run. It also prints whether `~/.bash_profile`, `~/.bash_login`, or `~/.profile` is supplying the bash login startup path before it verifies `kimi`, `codex`, and `claude`, and it reports whether Codex auth is coming from `OPENAI_API_KEY`, Codex CLI login, or both in that shared Kimi-backed shell.
