...
From the repo root, `make inspect-local`, `make doctor-local`, `make smoke-local`, and `make check-local` wrap the same bundled Kimi-backed workflow with `python3 -m agentflow ...`. If you do not need the Makefile wrapper, `agentflow check-local` is the direct CLI equivalent of `make check-local`, and its stderr preflight report now follows the requested run output style: summary for `--output summary`, JSON for `--output json`, and JSON for `--output json-summary`.
